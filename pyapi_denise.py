...
try:
    ...
    print('Failed to import segyio! Do pip install segyio, to enable all funcitons.')
...
class Denise(object):
    ...
    def _write_taper(self):
        ...
        if self.SWS_TAPER_FILE:
            ...
            if is_defined(self.taper.vp):
                _write_binary(self.taper.vp, self.TFILE + '.bin')
            if is_defined(self.taper.vs):
                _write_binary(self.taper.vs, self.TFILE + '_u.bin')
            if is_defined(self.taper.rho):
                _write_binary(self.taper.rho, self.TFILE + '_rho.bin')
    ...
    def _calc_nt_dt(self):
        ...
        maxrho = np.max(self.model.rho[np.nonzero(self.model.rho)])
        if is_defined(self.model.vs):
            ...
            if (np.nonzero(self.model.vs)[0] ).size==0:     maxvs=0
            else:   maxvs = np.max(self.model.vs[np.nonzero(self.model.vs)])
        else:
            maxvp = maxvs
        # set box constraints maximum precisely
        VPUPPERLIM=maxvp+1000
        VSUPPERLIM=maxvs+500
        RHOUPPERLIM=maxrho+500
        # Compute time step so it is consistent with seismic records and box constraints in future fwi simulations
        DT1 = self._check_stability(maxvp,maxvs)
        DT2 = self._check_stability(VPUPPERLIM,VSUPPERLIM)
        print('dt from true velocity models',DT1)
        print('dt from box constraints',DT2) 
        self.DT = self._check_stability(VPUPPERLIM,VSUPPERLIM)
    ...
    def _engine(self, model, src, rec, run_command, disable,_write_acquisition_flag):
    ...
    def forward(self, model, src=None, rec=None, run_command=None, disable=False,_write_acquisition_flag=1):
        """ Run forward modeling """
        self.MODE = 0
        # self.set_paths(makedirs=True)
        self._engine(model, src, rec, run_command,disable,_write_acquisition_flag=_write_acquisition_flag)

    def fwi(self, model, src=None, rec=None, run_command=None, disable=False,_write_acquisition_flag=1):
    ...
    def _get_filenames(self, dir, keys=None):
        ...
        # os.listdir('/lustre/project/k1404/pavel/DENISE-Black-Edition/fwi/multi/multi_cnn_15_weight_1351_7/model__Marmousi_f_z/stage0/fld/su')
    ...
    def get_shots_from_datadir(self, idx=None, keys=None, return_filenames=False):
        """ Reads data from self.savefolder/su/
        ...
        keys.append('.su')
        ###############################################
        self.DATA_DIR.split('/')[0:-2]
        data_directory=''
        for ii in self.DATA_DIR.split('/')[0:-1]:
            data_directory=data_directory+ii+'/'
        data_directory=data_directory[0:-1]
        files=self._get_filenames(data_directory,keys)
        ###############################################
        # files = self._get_filenames(self._root_su, keys)
        ###############################################
        if idx is not None:
    ...
    def get_shots_from_directory(self,directory,idx=None, keys=None, return_filenames=False):
        """ Reads data from self.savefolder/su/
        ...
        #     data_directory=data_directory+ii+'/'
        # data_directory=data_directory[0:-1]
        files=self._get_filenames(directory,keys)
        ###############################################
        # files = self._get_filenames(self._root_su, keys)
        ###############################################
        if idx is not None:
    ...
    def _parse_inp_file(self,**kwargs):
        ...
        else:   fname = os.path.join(self._root_denise, 'par/', 'DENISE_marm_OBC.inp')
        msg_report.append(f'==============================\nReport parsing {fname}...')
        msg_report.append(['LINE', 'ARG', 'VAL', 'TEXT'])
        self._print_1(f'Parse {fname}')
        # print('opening file from',os.getcwd())
    ...
    def _write_inp_file(self, write=True):
        ...
        if write:
            c = ''.join(c)
            print('parameters file name=',self.filename)
    ...
    def _write_src_rec(self, src_, rec_):
        ...
        np.save(f"{'/'.join(save_location)}/src_x.npy", src_.x)
        np.save(f"{'/'.join(save_location)}/src_y.npy", src_.y)
    ...
    def _check_max_frequency(self):
        ...
        if is_defined(self.model.vs):
            ...
            if (np.nonzero(self.model.vs)[0] ).size==0:     minvs=0
            else:   minvs = np.min(vs[np.nonzero(vs)])
    ...
    def add_fwi_stage(self, pro=0.01, time_filt=1, fc_low=0.0, fc_high=5.0, order=6, time_win=0, gamma=20,
        ...
        Args:
            ...
            offset_mute (int): Apply Offset mute to field and modelled seismograms
                ...
                OFFSET_MUTE = 2 - mute near-offset data for offset <= OFFSETC
            offsetc (float):
