...
from functions.utils.loaders import bandpass, butter_bandpass
...
def get_spectrum2_model(shot, dt=None, dx=None, fmax=None, kmax=None):
    ...
    if dt:
        ...
        shot = np.fft.fftshift(shot, -1)
        idx = np.abs(fax) < (fmax if fmax else np.max(fax))
        fax = fax[idx];
        shot = shot[:, idx]
    if dx:
...
def plot_spectrum2_model(shot, pclip=1.0, title='', colorbar=False, dt=None, dx=None, fmax=30, kmax=None,
    ...
    fft2_shot, fax, kax = get_spectrum2_model(shot, dt, dx, fmax, kmax)
    ...
    if dt or dx:
        kwargs['extent'] = [np.min(kax) if dx else 0, np.max(kax) if dx else nx, 
                            np.min(fax) if dt else 0, np.max(fax) if dt else nt]
        if dx: 
            plt.xlabel('Wavenumber, cycles / km') 
        else: 
            plt.gca().get_xaxis().set_visible(False) 
        if dt: 
            plt.ylabel('Wavenumber, cycles / km') 
        else: 
...
def plot_acquisition(v, dx, src=None, rec=None, title='', cax_label='', log=None, **kwargs):
    ...
    plt.figure(); ax = plt.gca(); im = ax.imshow(v/1000, **par); 
...
def plot_shot(shot, pclip=1.0, title='', colorbar=True, dt=None, dx=None, 
              figsize=None, axis=True, ax=None, cax_label='', 
              dpi=None,folder_path='pictures',file_path='1.png', **kwargs):
...
def plot_compare_stripes(shot_a, shot_b, width=20, step=20,folder_path='pictures',file_path='1.png', **kwargs):
    """
    Args:
    ...
    plot_shot(shot_plot,folder_path=folder_path,file_path=file_path,**kwargs)
