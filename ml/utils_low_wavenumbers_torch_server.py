...
def Plot_r2_losses_2(history, Title='Title', Save_pictures_path='./Pictures', Save_flag=0):
    ...
    plt.title('Average R2 misfit(prediction, target), '+Title+', train/val=' +
            numstr(loss_target_pred_train[-1])+'/'+numstr(loss_target_pred_valid[-1] ), fontsize=Fontsize)
    plt.legend([
...
def Plot_image(Data, Title='Title', c_lim='', x='', x_label='', y='', y_label='',
    ...
    if write_fig_title == 1:
        plt.title(Title)
    if Reverse_axis == 1:
        plt.imshow(np.flipud(Data), extent=extent,
            ...
            for count,ii in enumerate(original_sample):
                ...
                if count==0:    # choose x tensor
                    if inp_channels_type=='dm_i' or inp_channels_type=='m_i':
                        channels_to_process=range(augmented_tensor.shape[0])
                    ...
                    for i in channels_to_process:
                        ...
                        noise=(self.c*np.random.rand()*(dmax-dmin))*torch.rand(size=data.shape)
...
def sample_dataset(dataloader, generator, opt, epoch, flag_show_scaled_data=1, record_weights=0, data_mode='test'):
    ...
    for i, batch in enumerate(dataloader):
        ...
        titles=[
            ...
            'FWI result,'+'R2(FWI result,ideal_init_model)='+numstr(F_r2(fwi_result,ideal_init_model))+metric_name+numstr(chosen_metric(fwi_result,ideal_init_model)),
            'True model']
        PLOT_ML_Result_all_inputs(pics, numstr(chosen_metric(predicted_init_model,true_model)),history_flag=0,
            history=None, Boundaries=[], save_file_path=picture_name,
            dx=dx,dy=dz,Title=titles, Title2='',Show_flag=0,Save_flag=1,adaptive_colorbar=3)
        ###############
        if record_weights==1:
...
# else:
#     # tmp=img_B[0,:,:];   tmp=np.expand_dims(tmp,axis=0)
#     # np.expand_dims(img_B[0,:,:],axis=0)
#     # tmp=np.expand_dims(img_B[0,:,:],axis=0).cuda()
#     # tmp=np.expand_dims(img_B[:,0,:,:].cuda(),axis=0)
#     # tmp2=np.expand_dims(img_B[:,0,:,:].cuda(),axis=1)
#     # tmp3=torch.unsqueeze(img_B[:,0,:,:],dim=1)
#     # img_input=torch.cat((img_A,tmp3),1)
#     img_input=torch.cat((img_A,img_B),1)
...
#         self.to_fake_B_temp=[self.list_IDs[k] for k in indexes]
#         # Generate data
#         if self.to_fit:
...
# Plot_image(real_A[0,0,:,:].cpu().data.numpy().squeeze(),Show_flag=1,Save_flag=1,Title='x_ch1',
#     Aspect='equal',Save_pictures_path=opt.save_path,Reverse_axis=0)
# Plot_image(real_A[0,1,:,:].cpu().data.numpy().squeeze(),Show_flag=1,Save_flag=1,Title='x_ch2',
#     Aspect='equal',Save_pictures_path=opt.save_path,Reverse_axis=0)
# Plot_image(real_A[0,2,:,:].cpu().data.numpy().squeeze(),Show_flag=1,Save_flag=1,Title='x_ch3',
#     Aspect='equal',Save_pictures_path=opt.save_path,Reverse_axis=0)
# Plot_image(real_B[0,0,:,:].cpu().data.numpy().squeeze(),Show_flag=1,Save_flag=1,Title='t',
#     Aspect='equal',Save_pictures_path=opt.save_path,Reverse_axis=0)
# Plot_image(fake_B[0,0,:,:].cpu().data.numpy().squeeze(),Show_flag=1,Save_flag=1,Title='prediction',
#     Aspect='equal',Save_pictures_path=opt.save_path,Reverse_axis=0)
# Plot_image(M1.squeeze().T,Show_flag=1,Save_flag=1,Title='M1',
#     Aspect='equal',Save_pictures_path=opt.save_path)
# Plot_image(M2.squeeze().T,Show_flag=1,Save_flag=1,Title='M2',
#     Aspect='equal',Save_pictures_path=opt.save_path)
# Plot_image(M3.squeeze().T,Show_flag=1,Save_flag=1,Title='M3',
#     Aspect='equal',Save_pictures_path=opt.save_path)
...
#             # self.files=opt.path_test+opt.path_train[0:3]+opt.path_valid[0:1]
#             self.files=opt.path_test
#         elif mode == "all":self.files=opt.path_test+opt.path_train+opt.path_valid
#         elif mode == None:
#             self.files=file_list
#     def calculate_max_model_init(self):
#         const=0
...
#         with open(self.files[index % len(self.files)],'rb') as f:
#             data=np.load(f,allow_pickle=True)
#             x=data['input_data'];   #print('data[input_data]=',data['input_data'].shape)
#             t=data['output_data']
#             # t=data['output_data_smooth_100']
#             models_init=data['models_init']
#             dz=data['dz'];  dx=data['dx'];
#             data.close()
#         t=np.squeeze(t)
#         if x.ndim==4:   x=np.squeeze(x,axis=3)
#         nch=x.shape[0];nx=x.shape[1];nz=x.shape[2]
#         # print('nch=',nch)
#         # if self.load_precondition_data==1:
#         #     coefs=(1,0.05);     geom_spreading_matrix=(coefs[0]+coefs[1]*np.repeat(np.expand_dims(np.arange(0,nz),axis=0),nx,axis=0))
#         #     print('np.max(original x)=',np.max(x))
#         #     x=x*geom_spreading_matrix
...
#         ########    Add extra channels  ?
#         # models_init=np.squeeze(models_init)
#         # models_init=(models_init-1500)/self.max_for_initial_models
#         # zz = np.arange(nz) * dz
#         # zz = np.tile(zz,(nx,1))
#         # depth_matrix=zz
#         # depth_matrix=depth_matrix/depth_matrix.max()
#         # depth_matrix=imresize(depth_matrix,[self.opt.img_height,self.opt.img_width])
#         # models_init=imresize(models_init,[self.opt.img_height,self.opt.img_width])
#         ########
#         if is_defined(self.opt.inp_channels_type):
#             if self.opt.inp_channels_type:
#                 type_of_input=self.opt.inp_channels_type
#         if self.opt.channels==1:    type_of_input='1dv'
#         if self.opt.channels==2:    type_of_input='1grad_1dv'
#         # else:   type_of_input='allgrad_1dv'
#         # type_of_input='allgrad_1dv'
#         # if self.opt.channels==1:
#         if type_of_input=='1dv':
#             img_A=np.zeros((1,self.opt.img_height,self.opt.img_width))
#             img_A[0,:,:]=x_[-1,::]
#         elif type_of_input=='1grad_1dv':
#             img_A=np.zeros((2,self.opt.img_height,self.opt.img_width))
#             img_A[0,:,:]=x_[-2,::]
#             img_A[1,:,:]=x_[-1,::]
#         elif type_of_input=='allgrad_1dv':
#             img_A=x_[-self.opt.channels:,::]
#         elif type_of_input=='only_grads':
#             x_=x_[0:-1,::]  #select only gradients for input
#             # print('x_ cropped=',x_.shape)
#             # step=math.ceil(x_.shape[0]//self.opt.channels); print('step=',step)
#             # img_A=x_[::step,::]    #variant 1\
#             selected_channels=[0,5,10,15,20,25,30,35,40,45,49]
#             img_A=x_[selected_channels,::]    #variant 1
#             # print('img_A=',img_A.shape)
#         img_B=np.zeros((1,self.opt.img_height,self.opt.img_width))
#         img_B[0,:,:]=t
#         img_A_has_nan = np.isnan(img_A).any()
#         img_B_has_nan = np.isnan(img_B).any()
#         if img_A_has_nan:
#             # Plot_image(img_A.squeeze().T,Show_flag=1,Save_flag=1,Title='input_data_'+path.split('/')[-1].split('.')[0],dx=20,dy=20)
#             print('img_A_has_nan')
#         if img_B_has_nan:
#             print('img_B_has_nan')
#         return {"A": img_A,"B": img_B}
#         # if self.opt.channels==3:
#         #     img_A=np.zeros((self.opt.channels,self.opt.img_height,self.opt.img_width))
#         #     img_A[0,:,:]=x_
#         #     img_A[1,:,:]=models_init    # 0 to 1, (- min)/max
#         #     img_A[2,:,:]=depth_matrix
#     def __len__(self):
#         return len(self.files)

# r2_3=scores.sum()/batch_size
# v1 = y_pred.flatten()
# v2 = y.flatten()
# r2_2 = r2_score(v1, v2)
...
#     data = np.reshape(
#         data, (int(data.size/scaler.n_features_in_), scaler.n_features_in_))
...
# def scaling_data_01(data, preconditioning=True):
#     data_ = np.squeeze(data)
#     Nx = data_.shape[0]
#     Nz = data_.shape[1]
#     const1 = np.abs(np.min(data_))
#     data_ = data_+const1
#     const2 = np.max(np.abs(data_))
#     data_ = data_/const2
#     if preconditioning == True:
#         coefs = (1, 0.05)
#         geom_spreading_matrix = (
#             coefs[0]+coefs[1]*np.repeat(np.expand_dims(np.arange(0, Nz), axis=0), Nx, axis=0))
#         data_ = data_*geom_spreading_matrix
#         const3 = np.max(np.abs(data_))
#         data_ = data_/const3
...
#     if Nch == 1 and scaler.ndim > 1 and scaler.shape[0] > 1:
#         scaler = scaler[-1, :]
#     data_ = np.squeeze(data)
...
# def extract_xt_to_list2(list_of_structures,field):
#     list_of_arrays=[]
#     shape_= list( list_of_structures[0][field].shape  )
#     shape_.insert(0,len(list_of_structures))
#     output=np.empty(shape_)
#     for i in range(len(list_of_structures)):
#         output[i,::]=list_of_structures[i][field]
#         # list_of_arrays.append( list_of_structures[i][field] )
#     return output
