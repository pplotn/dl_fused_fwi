...
# scp -r ./logs/log1207/predictions_1207 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1207
# scp -r ./logs/log1220/predictions_1220 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1220
# scp -r ./logs/log1227/predictions_1227 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1227
# scp -r ./logs/log1230/predictions_1230 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1230
# scp -r ./logs/log1235/predictions_1235 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1235
# scp -r ./logs/log1239/predictions_1239 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1239
# scp -r ./logs/log1251/predictions_1251 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1251
# scp -r ./logs/log1273/predictions_1273 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1273
# scp -r ./logs/log1294/predictions_1294 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1294
# scp -r ./logs/log1350/predictions_1350 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1350
# scp -r ./logs/log1351/predictions_1351 /var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/predictions/predictions_1351
...
# dataset_path=['/var/remote/lustre/project/k1404/pavel/DENISE-Black-Edition/datasets/cgg_real_data/cnn_14_1_3_21_08']
###########################
...
inp_channels_type='dm_i'
# inp_channels_type='m_i'
# inp_channels_type='1dv_1init'
# inp_channels_type='1dv_1init_1sign'
# inp_channels_type='dm_i_1init_1sign'
inp_channels_type='dm_i_1init_1sign_1taper'
...
parser.add_argument("--first_checkpoint_interval", type=int,  default=10, help="interval between model checkpoints before epoch 50")
...
parser.add_argument("--noise_level",type=int,default=0.3,help="level of noise added to x data")
parser.add_argument("--dropout_value",type=int,default=0.5,help="dropout_value in conv_block3")
parser.add_argument("--number_of_filters",type=int,default=110,help="number of filters in convolution blocks")
...
######################################   load dataset paths
# if opt.n_epochs==-1:
#     opt.start_with_certain_dataset=1
...
tracking_train_dataloader2=DataLoader(ImageDataset(opt,
    mode=None,file_list=opt.path_train[0:1]),     #+path_test[-2:-1]+path_test[-4:-3]
    batch_size=1,shuffle=False,num_workers=1)
...
# sample_dataset(test_dataloader,generator,opt,opt.n_epochs,flag_show_scaled_data=1,data_mode='test',record_weights=0)
# exit()
...
if opt.dataloading_pipeline=='matrix' and opt.n_epochs!=-1:
    ...
    if single_thread_loading==0:
        ...
        temp = partial(extract_ml_data_to_list,opt)
        ...
        for path_ in opt.path_train:
            output_train.append(extract_ml_data_to_list(opt,path_))
        for path_ in opt.path_valid:
            output_valid.append(extract_ml_data_to_list(opt,path_))
...
if opt.augment_training_dataset==True and opt.n_epochs!=-1:
    ...
    train_dataset_from_array3=torch.utils.data.ConcatDataset([train_dataset_from_array,train_dataset_from_array2])
    ...
    valid_dataset_from_array3=torch.utils.data.ConcatDataset([valid_dataset_from_array,valid_dataset_from_array2])
        ...
        for phase in phases:
            ...
            if opt.dataloading_pipeline=='matrix':
                ...
                for x_batch,y_batch,taper_batch in dataloader:
                    ...
                    #     real_A[-1,0,::].cpu().detach().numpy()+real_A[-1,1,::].cpu().detach().numpy() ),axis=1).T,
            ...
            if phase=='training':
                ...
                if epoch<=50:    current_checkpoint_interval=opt.first_checkpoint_interval;current_plotting_interval=current_checkpoint_interval
                ...
                # if epoch%15==0 and epoch!=0:     #   opt.plotting_interval
                    ...
                    if opt.plot_samples==1:
                        ...
                        sample_dataset(tracking_train_dataloader2,generator,opt,opt.n_epochs,flag_show_scaled_data=1,data_mode='tracking_train',record_weights=0)
                        sample_dataset(tracking_val_dataloader2,generator,opt,opt.n_epochs,flag_show_scaled_data=1,data_mode='tracking_val',record_weights=0)
...
if opt.plot_samples==1:
    ...
    sample_dataset(tracking_train_dataloader,generator,opt,opt.n_epochs,flag_show_scaled_data=1,data_mode='tracking_train',record_weights=0)
    # sample_dataset(tracking_val_dataloader,generator,opt,opt.n_epochs,flag_show_scaled_data=1,data_mode='tracking_val',record_weights=0)
#########  save training session parameters to the file
with open(opt.save_path+'/'+'opt.txt', 'w') as f:
    json.dump(opt.__dict__, f, indent=2)
#########
calculate_prediction_histograms=0
if calculate_prediction_histograms==1:
    ...
    for phase in phases:
        ...
        for i,batch in enumerate(dataloader):
            ...
            predictions[ ((i)*batch_size):((i)*batch_size)+len(target_pred_acc),1]=np.nan
            predictions[ ((i)*batch_size):((i)*batch_size)+len(target_pred_acc),2]=np.nan
...
if plot_prediction_histograms==1:
    ...
    # scores_path='./logs/log1288'
    # scores_path='./logs/log27'
    ...
    dl_train_worst=DataLoader(ImageDataset(opt,transforms_=transforms_,
        ...
        batch_size=1,shuffle=False,num_workers=1)
    #################   R2 scores
    score={ 'train':np.mean(predictions_train.target_pred_acc.to_numpy()),
            'valid':np.mean(predictions_valid.target_pred_acc.to_numpy()),
            'test':np.mean(predictions_test.target_pred_acc.to_numpy())}
    Fontsize=17
    ...
    plt.hist((predictions_train.target_pred_acc.to_numpy()).T,
        range=[np.min(predictions_train.target_pred_acc), 1], bins=20, cumulative=False)    # range=[-2,1], bins=20, cumulative=False)
    plt.ylabel('Number of samples')
    plt.xlabel('R2(predicted,true)')
    plt.title('Training,total#='+str(predictions_train.shape[0])+',prediction score(train/val)='+numstr(score['train'])+'/'+numstr(score['valid']),fontsize=16)
    ...
    plt.figure(figsize=(10,8),dpi=300)
    plt.hist((predictions_valid.target_pred_acc.to_numpy()).T,range=[np.min(predictions_valid.target_pred_acc), 1], bins=20, cumulative=False)
    plt.ylabel('Number of samples')
    plt.xlabel('R2(predicted,true)')
    plt.title('Validation,total#='+str(predictions_valid.shape[0])+',prediction score(train/val)='+numstr(score['train'])+'/'+numstr(score['valid']),fontsize=16  )
    plt.legend(['target_pred_acc','init_model_pred_acc','init_model_pred_acc-init_model_inversion_acc'])
    ...
    plt.figure(figsize=(10,8),dpi=300)
    plt.hist((predictions_test.target_pred_acc.to_numpy()).T,range=[np.min(predictions_test.target_pred_acc), 1], bins=20, cumulative=False)
    plt.ylabel('Number of samples')
    plt.xlabel('R2(predicted,true)')
