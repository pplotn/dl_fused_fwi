...
# dataset_path=['/home/plotnips/Dropbox/Log_extrapolation/scripts/DENISE-Black-Edition-master/datasets/test']
###########################
...
parser.add_argument("--cnn_dropout",type=int,default=0.0,help="dropout in cnn") #   0.05,0.2
...
test_dataset = ImageDataset(opt, transforms_=transforms_, mode="test")

### Apply augmentation
# train_dataset = FlipLoader(train_dataset, 0.5)
# val_dataset = FlipLoader(val_dataset, 0)
# test_dataset = FlipLoader(test_dataset, 0.)
...
tracking_train_dataloader=DataLoader(ImageDataset(opt,transforms_=transforms_,
    mode=None,file_list=opt.path_train[0:6]),     #+path_test[-2:-1]+path_test[-4:-3]
    batch_size=1,shuffle=False,num_workers=1)
tracking_val_dataloader=DataLoader(ImageDataset(opt,transforms_=transforms_,
    mode=None,file_list=opt.path_valid[0:5]),
    batch_size=1,shuffle=False,num_workers=1)
opt.logging_loss_batch_size=len(train_dataloader)
# sample_models(train_dataloader,generator,opt,opt.n_epochs,flag_show_scaled_data=0)
    ...
    for epoch in range(0,opt.n_epochs):
        for phase in phases:
            ...
            if phase=='training':
                ...
                if epoch<15:    current_checkpoint_interval=20;current_plotting_interval=current_checkpoint_interval
                else:           current_checkpoint_interval=opt.checkpoint_interval;current_plotting_interval=opt.plotting_interval

                if epoch%current_plotting_interval==0 and epoch!=0==0:     #   opt.plotting_interval
